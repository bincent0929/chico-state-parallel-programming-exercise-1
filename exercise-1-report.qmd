...
I created a copy of the `sharpen.c` program and added OpenMP, set it to use 4 threads, and placed the pragma where recommended to try it, and it didn't really change the outcome. Here's the output:
...
The outcome is `{python} f'{ompsharpenc_fps:.2f}'` FPS again when rounded to the same degree.
