...
threads = 4
ompdct2_parallelized_portion = (1 - (1 / omp_dct_speedup)) / (1 - (1 / 4))
```
...
# System and measured data
my_system_cores = 8
threads_used = 12
actual_speedup = sharpen_grid_runtime_vs_nparallel_speedup

seq_time = 11.696272
par_time = 2.031316

# Calculate parallel fraction from actual data
# Using effective cores (8) since 12 threads > 8 cores
f = (my_system_cores * (actual_speedup - 1)) / (actual_speedup * (my_system_cores - 1))

# Generate thread counts
threads = np.arange(1, 13)

# Linear ideal (capped at core count)
linear_ideal = np.minimum(threads, my_system_cores)

# Amdahl's law curve
amdahl_speedup = 1 / ((1 - f) + f / np.minimum(threads, my_system_cores))

# Plot
plt.figure(figsize=(10, 6))
plt.plot(threads, linear_ideal, 'b--', label='Linear Ideal (capped at 8 cores)', linewidth=2)
plt.plot(threads, amdahl_speedup, 'g-', label=f'Amdahl\'s Law (f={f:.3f})', linewidth=2)
plt.plot(threads_used, actual_speedup, 'ro', markersize=10, label=f'Actual (12 threads, {actual_speedup:.2f}x)')
plt.axhline(y=my_system_cores, color='gray', linestyle=':', alpha=0.7, label='Core limit')

plt.xlabel('Number of Threads', fontsize=12)
plt.ylabel('Speedup', fontsize=12)
plt.title('Speed-up Comparison: Linear Ideal vs Amdahl\'s Law vs Actual\n(8-core system, 4x3 thread grid)', fontsize=14)
