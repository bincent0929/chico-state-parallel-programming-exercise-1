...
The output looked a little weird to me when I opened it with an image viewer. So I tried to lower the iterations to `20` iterations, but it still came out looking weird. From `20` iterations I got `44.310001 FPS`. \##### shared.c I got this when I ran the non-OpenMP `sharpen.c`:
...
sharpenc_fps = sharpenc_frames / (sharpenc_stop - sharpenc_start)
```

From that calculation I got: `{python} f'{sharpenc_fps:.2f}'` FPS.
...
ompsharpenc_start = 0.021750
ompsharpenc_stop = 11.689725
ompsharpenc_fps = ompsharpenc_frames / (ompsharpenc_stop - ompsharpenc_start)
```

Rounded to the same place as the last outcome, it's `{python} f'{ompsharpenc_fps:.2f}'` FPS again.
