...
import numpy as np
import matplotlib.pyplot as plt
# experimental & env data
S = 8

# finding the average $time optimized$ between the two parallelization methods
P_omp = (1 - 1/omp_moret_4_vs_nparallel_speedup) / (1 - (1 / S))
P_pth = (1 - 1/sharpen_grid_runtime_vs_nparallel_speedup) / (1 - (1 / S))
P_overall = (P_omp + P_pth) / 2

# Amdahls law and Linear Ideal Speed-Up
N = np.arange(1, S + 1)
linear_ideal = N
amdahls = 1 / ((1 - P_overall) + (P_overall / N))

# plot
plt.figure()
plt.plot(N, linear_ideal, label="Linear Ideal")
plt.plot(N, amdahls, label=f"Amdahls")
plt.scatter([1], [1.0], label="Single")
plt.scatter([S], [omp_moret_4_vs_nparallel_speedup], label=f"OpenMP")
plt.scatter([S], [sharpen_grid_runtime_vs_nparallel_speedup], label=f"PThreaded")
