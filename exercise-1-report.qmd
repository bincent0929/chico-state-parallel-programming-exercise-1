...
### b): After compiling `dct2.c` and running it, it runs for a little while and then terminates with no output.
...
As I calculated earlier $SU_{max} = T_{seq} / T_{par} \approx$ `{python} f'{omp_dct_speedup:.2f}'`

Now, I'll use that in Amdahl's equation but I'll rearrange it to present the proportion of code that is being parallelized.

``` {python}
# the proportion of the program that is paralleized according to Amdahl's law
ompdct2_threads = 4
ompdct2_parallelized_portion = (1 - (1 / omp_dct_speedup)) / (1 - (1 / ompdct2_threads))
```

By my calculations, the portion of the program that was parallelized was `{python} f'{(ompdct2_parallelized_portion * 100):.2f}%'` leaving `{python} f'{((1 - ompdct2_parallelized_portion) * 100):.2f}%'` that wasn't parallelized, according to Amdahl's Law.
...
In reading Chapter 24 from [here](http://www.dspguide.com/ch24.htm), very *roughly* in my own words, PSF convolution goes through the image as a matrix using another matrix that initially is composed of all 0's other than the value at `row = 0` and `column = 0`. In doing so, the value in the matrix that is nonzero ends of "spreading out" into a new pattern in the matrix.
...
Even when I change the amount of threads that the OpenMP program can use, it seems to cap out at around a 2.90 seconds runtime. Which still comes out with the PThreads implementation resulting in a `{python} f'{pthreads_vs_omp_moret_4_speedup:.2f}'`x speedup vs. OpenMP.

``` {python}
omp_moret_4_vs_nparallel_speedup = sharpenc_runtime / ompsharpenc_runtime_moret_4
sharpen_grid_runtime_vs_nparallel_speedup = sharpenc_runtime / sharpen_grid_runtime
```

In terms of how the two compare in speedup vs. the non-parallelized implementation, `ompsharpen.c` is `{python} f'{omp_moret_4_vs_nparallel_speedup:.2f}'`x times faster and `sharpen_grid.c` is `{python} f'{sharpen_grid_runtime_vs_nparallel_speedup:.2f}'`x times faster.

I do believe that if I added a `collapse` to my program that the OpenMP program will converge closer in time to the PThreads implementation.

#### By Amdahl's Law And Linear Ideal Speedup

Now I'll use the results I got in Amdah's Law and Linear Ideal Speedup.

According to running `lscpu`, I have 8 CPU(s) to work with:
```bash
CPU(s):                   8
...
# finding the average $time optimized$ between the two parallelization methods
threads = 8
time_optimized_omp = (1 - 1/omp_moret_4_vs_nparallel_speedup) / (1 - (1 / threads))
time_optimized_pthreads = (1 - 1/sharpen_grid_runtime_vs_nparallel_speedup) / (1 - (1 / threads))
time_optimized_average = (time_optimized_omp + time_optimized_pthreads) / 2

N = np.arange(1, threads + 1)
linear_ideal_values = N
amdahls_values = 1 / ((1 - time_optimized_average) + (time_optimized_average / N))

plt.figure()
plt.plot(N, linear_ideal_values, label="Linear Ideal Prediction")
plt.plot(N, amdahls_values, label=f"Amdahl's Prediction")
plt.scatter([1], [1.0], label="Single-Threaded")
plt.scatter([threads], [omp_moret_4_vs_nparallel_speedup], label=f"Recorded OpenMP Speedup")
plt.scatter([threads], [sharpen_grid_runtime_vs_nparallel_speedup], label=f"Recorded PThreaded Speedup")
plt.xlabel("Threads")
plt.ylabel("Speedup")
plt.xticks(N)
plt.grid(True)
